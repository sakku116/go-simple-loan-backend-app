...
func NewMySqlDB() *gorm.DB {
	...
	logger.Debugf("ensuring database: %s", Envs.MYSQL_DATABASE)
	err = DB.Exec(fmt.Sprintf("CREATE DATABASE IF NOT EXISTS `%s`", Envs.MYSQL_DATABASE)).Error
	if err != nil {
	...
	logger.Debugf("connecting to database: %s", Envs.MYSQL_DATABASE)
	dsn = fmt.Sprintf(
		"%s:%s@tcp(%s:%v)/%s?charset=utf8mb4&parseTime=True&loc=Local",
		Envs.MYSQL_USER,
		Envs.MYSQL_PASSWORD,
